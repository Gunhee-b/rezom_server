...
export function AnalyzeWorldTop5Page() {
  ...
  const loadQuestions = async () => {
    ...
    try {
      ...
      try {
        const top5Response = await fetch(`http://localhost:3000/define/social-analysis/top5`, {
          credentials: 'include'
        });
        if (top5Response.ok) {
  ...
  const saveTop5 = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/admin/define/social-analysis/top5`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`,
      ...
      if (response.ok) {
        ...
        alert('Failed to save Top-5: Please check the console for details.');
      }
