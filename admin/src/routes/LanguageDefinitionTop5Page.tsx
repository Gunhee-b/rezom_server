...
export function LanguageDefinitionTop5Page() {
  ...
  const loadQuestions = async () => {
    setLoading(true);
    try {
      // Load questions from the language-definition category (categoryId: 1)
      ...
      // Load existing top5 for language-definition
      try {
        const top5Data = await api('/define/language-definition/top5');
        ...
        console.log('No existing top5 data for language-definition');
  ...
  const saveTop5 = async () => {
    ...
    try {
      ...
      await api('/admin/define/language-definition/top5', {
        method: 'POST',
        accessToken,
        json: { questionIds }
      });
      
      alert('Language Definition Top-5 questions updated successfully!');
      await loadQuestions();
  ...
  return (
    <AdminLayout title="Language Definition Top-5 Manager">
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Language Definition Top-5</h1>
            <p className="text-gray-600 mt-1">
              Manage the top 5 questions for language definition category
            </p>
          </div>
          <button 
            onClick={saveTop5}
        ...
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded">
          <p className="text-sm text-blue-800">
            <strong>Current Language Definition Top-5:</strong> Questions with IDs {selectedQuestions.map(q => q.id).join(', ')} 
          </p>
          <p className="text-xs text-blue-600 mt-1">
            Endpoint: POST /admin/define/language-definition/top5
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white shadow rounded-lg">
            <div className="px-6 py-4 border-b">
              <h2 className="text-lg font-medium">Available Language Definition Questions</h2>
            </div>
          ...
          <div className="bg-white shadow rounded-lg">
            <div className="px-6 py-4 border-b">
              <h2 className="text-lg font-medium">Language Definition Top-5 ({selectedQuestions.length}/5)</h2>
            </div>
            <div className="p-4">
