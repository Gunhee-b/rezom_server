        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          {dailyData?.question ? (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <h3 className="font-semibold text-green-900 mb-2">{(dailyData.question as any)?.title}</h3>
              <p className="text-sm text-green-700">{(dailyData.question as any)?.body}</p>
              <div className="mt-2 text-xs text-green-600">
                Question ID: {(dailyData.question as any)?.id}
              </div>
            </div>
          ) : (
              ...
              {questions.map((question: Question) => (
                <div
                  ...
                  className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                    selectedQuestionId === (question as any)?.id
                      ? 'border-blue-500 bg-blue-50'
                      : (dailyData?.question as any)?.id === (question as any)?.id
                      ? 'border-green-500 bg-green-50'
                      : 'border-gray-200 hover:border-gray-300'
                  }`}
                  onClick={() => setSelectedQuestionId((question as any)?.id)}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <input
                          type="radio"
                          checked={selectedQuestionId === (question as any)?.id}
                          onChange={() => setSelectedQuestionId((question as any)?.id)}
                      ...
                      <div className="text-xs text-gray-400 ml-6 mt-1">
                        ID: {(question as any)?.id} | Created: {new Date(question.createdAt).toLocaleDateString()}
                      </div>
                    </div>
                  </div>
                </div>
