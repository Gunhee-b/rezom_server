...
import { updateAnswer, deleteAnswer } from '@/api/define'
...
export default function MyQuestionsPage() {
  ...
  const [deletingAnswer, setDeletingAnswer] = useState<UserAnswer | null>(null)
  ...
  const isLoading = questionsLoading || answersLoading || insightsLoading
  const hasError = questionsError || answersError || insightsError

  if (isLoading) return <div className="p-6">불러오는 중…</div>
  if (hasError) return <div className="p-6 text-red-600">목록을 가져오지 못했습니다.</div>

  const hasContent = (answers && answers.length > 0) || (insights && insights.length > 0)
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
            ...
            {answers.map((answer) => (
              <div key={answer.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  ...
                  >
                    {answer.title && (
                      <h3 className="font-semibold text-gray-900 mb-2 hover:text-emerald-600 transition-colors">{answer.title}</h3>
                    )}
      ...
      {editingAnswer && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4">
            <h2 className="text-xl font-semibold mb-4">답변 수정</h2>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                const formData = new FormData(e.currentTarget);
            ...
            >
              ...
              <div>
                ...
                <textarea
                  name="body"
                  defaultValue={editingAnswer.body}
                  rows={10}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 resize-none"
                  required
                />
              ...
              <div className="text-sm text-gray-500">
                질문: {editingAnswer.Question.title}
              </div>
              <div className="flex justify-end gap-3">
                <button
                  type="button"
                ...
                <button
                  ...
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-400 text-white rounded-lg transition-colors"
                >
                  {updateAnswerMutation.isPending ? '수정 중...' : '수정'}
                </button>
              </div>
            </form>
          </div>
      ...
      {deletingAnswer && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h2 className="text-xl font-semibold mb-4">답변 삭제</h2>
            <p className="text-gray-600 mb-6">
              정말로 이 답변을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.
            </p>
            <div className="bg-gray-50 p-3 rounded mb-4">
              <p className="text-sm text-gray-600">
                <span className="font-medium">질문:</span> {deletingAnswer.Question.title}
              </p>
              {deletingAnswer.title && (
                <p className="text-sm text-gray-600 mt-1">
                  <span className="font-medium">답변 제목:</span> {deletingAnswer.title}
                </p>
              )}
            </div>
            <div className="flex justify-end gap-3">
              <button
                onClick={() => setDeletingAnswer(null)}
                className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors"
              >
                취소
              </button>
              <button
                onClick={() => deleteAnswerMutation.mutate(deletingAnswer.id)}
